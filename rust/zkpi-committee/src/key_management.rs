...
/// Service-owned exclusive state-file lock shared by committee and venue stores.
...
/// Derive the existing encrypted-store key format from a passphrase and salt.
pub fn derive_secret_key(passphrase: &[u8], salt: &[u8]) -> Result<[u8; 32], String> {
    let mut key = [0_u8; 32];
...
/// Seal store bytes with authenticated context. The caller must use a fresh nonce.
...
/// Open store bytes only after authenticating the complete ciphertext and context.
