...
impl ProofStateStore {
    ...
    fn read(&self) -> Result<DurableProofState, String> {
        ...
        if value.get("version").and_then(Value::as_u64) != Some(u64::from(PROOF_STATE_VERSION)) {
            return Err(
                "proof state requires explicit schema migration; legacy state was preserved".into(),
            );
        }
...
struct ProofJob {
    ...
    limit_round1: Option<zkpi_proofs::threshold_range::RangeRound1Secret>,
    pool_remainder: Option<MpcLimitNode>,
    pool_remainder_bound: Option<BoundMpcLimitNode>,
    pool_remainder_round1: Option<zkpi_proofs::threshold_range::RangeRound1Secret>,
...
pub fn verify_peer_identity(
    ...
    let publication_public =
        hex::decode(&entry.publication_public).map_err(|_| "FROST publication key is malformed")?;
    ...
    identity
        ...
        .map_err(|_| "FROST exchange and publication keys lack their node identity signature")?;
    MlDsa65Verifier
        .verify(
...
impl ProofParty {
    ...
    fn manifest(
        ...
        if own.party != own_party
            ...
            || own.publication_public != hex::encode(self.application_identity.hybrid_public_key())
