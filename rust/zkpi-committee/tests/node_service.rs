...
use zkpi_committee::application_crypto::Signature;
...
use zkpi_committee::key_management::{create_ca, issue_mutual_tls_certificate, write_tls_bundle};
...
use zkpi_committee::wire::{Frame, PAYLOAD_BYTES};
use zkpi_proofs::kyb::{
...
fn real_mutual_tls_fixed_records_durable_idempotency_and_reconnect() {
    ...
    let closed = coordinator
        .call(&json!({"version": zkpi_committee::node_service::VERSION, "request_id": "close-9", "operation": "close_slot", "slot": 9}))
        .unwrap();
    ...
    assert!(
        admission_bytes > 4092 && admission_bytes <= zkpi_committee::node_service::MAX_JSON_BYTES
    );
    ...
    let job = json!({"version": zkpi_committee::node_service::VERSION, "request_id": "compute-9", "operation": "compute",
    ...
    assert!(
        execution_bytes > 4092 && execution_bytes <= zkpi_committee::node_service::MAX_JSON_BYTES
    );
    for offset in [14 + 1984, 14 + 1984 + 64] {
    ...
    server.stop();
    assert_eq!(
        zkpi_committee::node_service::encode_record(&json!({"ok": true}))
            .unwrap()
            .len(),
        RECORD_BYTES
    );
...
fn incomplete_duplicate_unclosed_and_unopened_slots_are_refused() {
    ...
    let short_close = coordinator
        .call(&json!({"version": zkpi_committee::node_service::VERSION, "request_id": "close-short", "operation": "close_slot", "slot": 40}))
        .unwrap();
    let short_compute = coordinator
        .call(&json!({"version": zkpi_committee::node_service::VERSION, "request_id": "compute-short", "operation": "compute", "slot": 40, "shape_digest": shape_digest.clone()}))
        .unwrap();
    ...
    let duplicate_close = coordinator
        .call(&json!({"version": zkpi_committee::node_service::VERSION, "request_id": "close-duplicate", "operation": "close_slot", "slot": 41}))
        .unwrap();
    ...
    let unclosed = coordinator
        .call(&json!({"version": zkpi_committee::node_service::VERSION, "request_id": "compute-unclosed", "operation": "compute", "slot": 42, "shape_digest": shape_digest}))
        .unwrap();
    let unopened = coordinator
        .call(&json!({"version": zkpi_committee::node_service::VERSION, "request_id": "compute-unopened", "operation": "compute", "slot": 43, "shape_digest": shape_digest}))
        .unwrap();
