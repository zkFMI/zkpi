...
use zkpi_committee::frost_coordinator::{finalize_frost_dkg, prepare_frost_dkg};
use zkpi_committee::proof_client::ProofPartyRpc;
use zkpi_committee::proof_party::{ProofParty, ProofPartyConfig, ProofRequest};
...
fn hybrid_dkg_envelopes_reject_downgrade_and_sender_substitution_before_commit() {
    ...
    use zkpi_committee::selective_disclosure::WinnerEnvelope;
    let root = TempDir::new().unwrap();
    let mut parties = (0_u16..7)
        .map(|node| LocalParty::new(node, root.path()))
        .collect::<Vec<_>>();
...
fn peer_identities_bind_the_publication_key_under_both_self_signatures() {
    use zkpi_committee::proof_party::{verify_peer_identity, FrostPeerEntry};
    let root = TempDir::new().unwrap();
    let mut parties = (0_u16..7)
        .map(|node| LocalParty::new(node, root.path()))
        .collect::<Vec<_>>();
