...
fn hybrid_delivery_refuses_downgrade_and_requires_all_components() {
    ...
    .unwrap();
    let open = |candidate: &zkpi_committee::selective_disclosure::WinnerEnvelope,
                key: &KeyRecord,
                now: u64| {
        open_if_winner(
            candidate,
    ...
    let decoded = zkpi_committee::selective_disclosure::WinnerEnvelope::decode(&encoded).unwrap();
    ...
    assert!(
        zkpi_committee::selective_disclosure::WinnerEnvelope::decode(&encoded[..encoded.len() - 1])
            .is_err()
    );
    let mut trailing = encoded.clone();
    trailing.push(0);
    assert!(zkpi_committee::selective_disclosure::WinnerEnvelope::decode(&trailing).is_err());
    let mut legacy = encoded.clone();
    legacy[..DOMAIN.len()].copy_from_slice(b"QOMM:WINNER:ENVELOPE:v2");
    legacy[DOMAIN.len()] = 2;
    assert!(
        zkpi_committee::selective_disclosure::WinnerEnvelope::decode(&legacy)
            .unwrap_err()
            .contains("v2")
    );
    assert_eq!(
