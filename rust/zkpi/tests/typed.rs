...
fn hybrid_payment_and_typed_wire_require_both_registered_quorums() {
    ...
    let legacy = zkpi::wire::encode(&instruction.payment);
    let signers = (1_u8..=7)
    ...
    assert_eq!(
        zkpi::wire::encode(&zkpi::wire::decode(&legacy).unwrap()),
        legacy
    );
    ...
    assert!(zkpi::wire::decode(&encoded[..encoded.len() - 1]).is_err());
